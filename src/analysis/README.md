...
narratives), combined into a single figure with a shared colorbar. Panels are also outlined by a
qualitative "reasoning strategy" grouping (`STRATEGY_MAPPING`) describing how each model's accuracy
tends to shift between quadrants in the asymmetric inference case.
...
Both scripts only depend on the paths listed in `QUARTERS` and the standard result-CSV schema
