...
a 2x2 heatmap panel per model, color-coded by reasoning strategy.
...
def _draw_panel(ax, model: str, quarters: dict, cmap, norm, first: bool) -> None:
    ...
    for r in range(2):
        for c in range(2):
            ...
            ax.add_patch(plt.Rectangle((c, 1 - r), 1, 1, color=color, ec="white", lw=2))
            text = f"{val:.0%}" if not np.isnan(val) else "N/A"
            ax.text(c + 0.5, 1 - r + 0.5, text, ha="center", va="center", fontsize=45, color="black")
    ...
    if color:
        ax.add_patch(mpatches.Rectangle((-0.03, -0.03), 2.06, 2.06, fill=False,
                                        edgecolor=color, lw=12, clip_on=False, zorder=10))
    ...
    ax.set_title(model, fontsize=45, weight="bold", pad=45, color="#1c1c1c", wrap=True)
...
def _add_colorbar_and_legend(fig, axes, cmap, norm) -> None:
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=axes, location="right", shrink=0.6, pad=0.01, aspect=40)
    cbar.set_ticks([0, 0.5, 1.0])
    cbar.set_ticklabels(["0%", "50%", "100%"])
    cbar.ax.tick_params(labelsize=40, length=0, colors="#1c1c1c")
    ...
    fig.legend(handles=legend_handles, loc="lower center", bbox_to_anchor=(0.5, -0.05),
               ...
               title_fontproperties={"size": 45, "weight": "bold"})
...
def main() -> None:
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "truncated_blues", plt.cm.Blues(np.linspace(0.0, 0.75, 256)))
    norm = mcolors.Normalize(vmin=0, vmax=1)

    for idx, (model, quarters) in enumerate(sorted_models):
        _draw_panel(axes[idx], model, quarters, cmap, norm, first=(idx == 0))
